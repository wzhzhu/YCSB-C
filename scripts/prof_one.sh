...
#
# Usage: prof_one.sh <scheme> <threads> <warmup_s> <duration_s> <tag> [cache_gb] [workload]
set -u
...
echo "[prof] launching scheme=$SCHEME threads=$THREADS workload=$WORKLOAD"
# 200M ops so the transaction phase far outlasts warmup+sampling; we kill it.
setsid python3 scripts/run_rocksdb_matrix.py \
  --workloads "$WORKLOAD" --schemes "$SCHEME" --threads "$THREADS" --cache-gb "$CACHE_GB" \
